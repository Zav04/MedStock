...
def GeneratePdfRealocacoes(self, table_widget: QTableWidget, file_path: str):
    width, height = A4
    c = canvas.Canvas(file_path, pagesize=A4)
    ...
    draw_header()
    c.drawString(60, height - 80, "Relatório de Realocações")

    table_data = [["Req. Origem", "Req. Destino", "Consumível", "Quantidade", "Data"]]
    for row in range(table_widget.rowCount()):
        row_data = [
            table_widget.item(row, 0).text(),
            table_widget.item(row, 1).text(),
